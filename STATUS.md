...
| Touchscreen | Atmel maXTouch MXT224S | Known | Downstream Express board data confirms Atmel `MXT224S` at `0x4a`, IRQ GPIO 11, 480x800 range, `I8730_AT_1226` firmware/config tag, and board-revision-dependent rails; mainline driver exists. | Add GSBI3 I2C and touchscreen/regulator nodes when the kernel tree is free. |
| Touchkeys | Cypress touchkey | Known | Downstream Express board data registers capacitive Cypress Menu/Back touchkeys at `0x20`, IRQ GPIO 65, plus LED/power control. Android overlay declares Home+Back+Menu+Volume hardware keys, so the missing visible legends are likely unlit capacitive keys rather than absent hardware. | Check mainline driver/binding match or add support when the kernel tree is free. |
