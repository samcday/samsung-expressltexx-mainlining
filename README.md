...
source breadcrumbs live in `RESEARCH.md`; this file is only the quick human
status page.
...
| Status | Meaning |
| --- | --- |
| Working | Present in `linux/`, built, and tested on hardware. |
| Builds | Present in `linux/` and build-tested, but not fully hardware-verified. |
| Partial | Some support exists, but important pieces are missing or deliberately hacked. |
| Known | Downstream facts are recorded, but no useful mainline node/driver wiring yet. |
| Blocked | Tried or investigated; needs a fix before it is useful. |
| Out of scope | Not a current bring-up target. |
