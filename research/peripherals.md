...
- Touchkeys are Cypress capacitive Menu/Back touchkeys on a bitbanged I2C bus ID 16, I2C address `0x20`, IRQ GPIO 65, keycodes `KEY_MENU` and `KEY_BACK`. Android declares hardware Home+Back+Menu+Volume keys, and downstream exposes `/sys/class/sec/sec_touchkey`, so the device appears to have capacitive touchkeys even if their legends/backlight are not currently visible. Newer revs use bitbang SDA/SCL GPIO 24/25 plus `8917_lvs5` for 1.8 V I/O, `8917_l30` set to 2.8 V for the MCU, and LED regulator `8917_l33` set to 3.3 V. Older revs use SDA/SCL GPIO 71/72, LDO enable GPIO 99, and LED GPIO 51. A mainline attempt to model newer-rev touchkeys with L30/L33 as RPM regulators caused an RPM fatal reset. The current mainline iteration keeps L30/L33 out of the RPM node and exposes them through a minimal PM8xxx SSBI direct regulator provider before wiring touchkey `vcc`/`vdd`. Hardware now confirms touchkey input events work and LED brightness works with 0/100 behavior.
...
- `linux/Documentation/devicetree/bindings/input/cypress,tm2-touchkey.yaml:20-53` allows `cypress,midas-touchkey`, `vcc-supply`, `vdd-supply`, `vddio-supply`, and `linux,keycodes`. `linux/drivers/input/keyboard/tm2-touchkey.c:63-68` defines the `cypress,midas-touchkey` register layout used for the Express Cypress protocol, and `tm2-touchkey.c:334-347` exposes the OF compatible table.
...
- `linux/arch/arm/boot/dts/qcom/qcom-msm8930-samsung-expressltexx.dts:78-101` models the newer-rev touchkey node on GPIO-bitbanged I2C GPIO 24/25 with IRQ GPIO 65, `KEY_MENU`/`KEY_BACK`, `vcc-supply = <&pm8917_l30>`, `vdd-supply = <&pm8917_l33>`, and `vddio-supply = <&pm8917_lvs5>`. `qcom-msm8930-samsung-expressltexx.dts:178-195` declares the direct PM8917 L30/L33 supplies; `qcom-msm8930-samsung-expressltexx.dts:228-240` declares the touchkey pin states. Hardware input testing confirms capacitive Menu/Back events reach the input subsystem, and LED testing confirms writing 100 lights the LEDs while writing 0 turns them off.
- `linux/arch/arm/boot/dts/qcom/qcom-msm8930-samsung-expressltexx.dts:9-214` currently models board identity, simple-framebuffer, TLMM GPIO keys, PM8917 power key, minimal PM8917 RPM USB/eMMC supplies, GSBI5 UART, SDCC1/eMMC, probe-only touchkeys, and USB peripheral mode.
- No mainline Expressltexx DT nodes currently model touchscreen, haptics, MUIC, NFC, sensors, MHL, display panel, cameras, audio, WLAN/Bluetooth/FM, charger/BMS, or battery profile.
...
- Good low-risk future DTS candidates after PMIC/regulator plumbing settles are maXTouch, YAS532/MPU sensors, and TAOS light/prox. MUIC/PN547/touchkeys/haptics may need driver or binding checks before they become clean upstream nodes.
