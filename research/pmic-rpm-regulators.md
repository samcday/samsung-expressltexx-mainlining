...
- PM8917 rails currently modeled through RPM in the board DT are S4, L3, L4, L5, L6, L7, LVS5, and LVS6. L3 supplies HSUSB 3.3 V at `3075000` uV, L4 supplies HSUSB 1.8 V at `1800000` uV, L5 supplies SDCC1 `sdc_vdd` at `2950000` uV, S4 supplies SDCC1 `sdc_vdd_io` at `1800000` uV, L6 supplies SDCC3 `sdc_vdd` at `2950000` uV, L7 supplies SDCC3 `sdc_vdd_io` at `1850000..2950000` uV, LVS5 is used as the touchkey I/O switch, and LVS6 is used as the maXTouch 1.8 V I/O switch. Hardware boot confirmed LVS5 removes the touchkey `vddio` dummy-regulator warning and LVS6 removes the maXTouch `vdd` deferred-probe warning. Do not model L30/L31/L33 as RPM rails: attempts that enabled these PLDOs through RPM caused `qcom_rpm 108000.rpm: RPM triggered fatal error` and reset the board. The current touchkey and touchscreen DT nodes wire L30/L31/L33 through the separate PM8xxx direct regulator provider instead.
- PM8917 direct L30/L31/L33 use downstream PM8921-core register addresses from the PM8917 extra regulator table: L30 control/test at `0x0a3`/`0x0a4`, L31 control/test at `0x0a5`/`0x0a6`, and L33 control/test at `0x0c6`/`0x0c7`. They are PM8xxx PLDOs with downstream constraints L30 `2800000` uV for newer-rev Cypress touchkey MCU power, L31 `3300000` uV for newer-rev Atmel maXTouch analog power, and L33 `3300000` uV for touchkey LED power. The mainline direct provider programs the PM8xxx LDO control/test registers through the existing PM8xxx SSBI regmap and supports only these three rails for now.
...
- `android_kernel_samsung_msm8930-common/arch/arm/mach-msm/board-8930-input-mxt.c:448-550` gets/enables `8917_lvs6` and `8917_l31` for newer-rev maXTouch power and sets `8917_l31` to `3300000` uV.
- `android_kernel_samsung_msm8930-common/arch/arm/mach-msm/board-8930-regulator-pm8917.c:617-694` defines L30/L31/L33 in the PM8xxx direct regulator pdata, not the RPM init table; `board-8930-regulator-pm8917.c:696-760` omits L30/L31/L33 from RPM-managed regulators and lists LVS5/LVS6 as RPM voltage switches supplied by `8917_s4`.
- `android_kernel_samsung_msm8930-common/drivers/mfd/pm8921-core.c:446-462` defines PM8917-specific direct regulators beyond PM8921, including L30 at `0x0a3`/`0x0a4`, L31 at `0x0a5`/`0x0a6`, and L33 at `0x0c6`/`0x0c7`.
...
- `linux/include/dt-bindings/mfd/qcom-rpm.h:173-185` assigns mainline resource IDs for the PM8038/PM8917 MSM8930 RPM rails currently modeled, including PM8917 LVS5/LVS6 but not L30/L31/L33.
- `linux/drivers/mfd/qcom_rpm.c:341-355` adds the MSM8930 RPM resource table with PM8917 LVS5/LVS6 target/status/selector IDs; `qcom_rpm.c:470` matches `qcom,rpm-msm8930`.
- `linux/drivers/regulator/qcom_rpm-regulator.c:918-953` adds `qcom,rpm-pm8038-regulators` and `qcom,rpm-pm8917-regulators` support; `qcom_rpm-regulator.c:927-936` registers PM8917 S4/L3/L4/L5/L6/L7/LVS5/LVS6.
- `linux/arch/arm/boot/dts/qcom/qcom-msm8930.dtsi:87-96` adds the MSM8930 RPM node.
- `linux/arch/arm/boot/dts/qcom/qcom-msm8930-samsung-expressltexx.dts:160-222` declares the PM8917 RPM S4/L3/L4/L5/L6/L7/LVS5/LVS6 regulators; `qcom-msm8930-samsung-expressltexx.dts:248-262` declares PM8xxx direct L30/L31/L33 regulators; `qcom-msm8930-samsung-expressltexx.dts:95-97` wires touchkey power to L30/L33/LVS5; `qcom-msm8930-samsung-expressltexx.dts:146-157` wires touchscreen power to L31/LVS6; `qcom-msm8930-samsung-expressltexx.dts:265-282` wires SDCC1/SDCC3 supplies; `qcom-msm8930-samsung-expressltexx.dts:405-407` wires the USB PHY to L4/L3.
- `linux/drivers/regulator/qcom-pm8917-regulator.c:45-49` defines the direct PM8917 PLDO voltage ranges; `qcom-pm8917-regulator.c:116-155` programs PM8xxx PLDO test-bank range/fine-step bits and control-register voltage bits; `qcom-pm8917-regulator.c:187-191` exposes only L30/L31/L33 with downstream-backed register addresses.
